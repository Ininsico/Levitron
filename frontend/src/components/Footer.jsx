...
export default function Footer() {
  return (
    <footer className="relative border-t border-ink-950/10 bg-cream-100">
      <div className="shell py-16">
        <div className="grid gap-12 lg:grid-cols-[1.4fr_repeat(3,1fr)]">
          ...
          {footerColumns.map((column) => (
            <div key={column.title}>
              <p className="text-xs font-semibold uppercase tracking-[0.14em] text-ink-400">{column.title}</p>
        ...
        <div className="flex flex-col items-start justify-between gap-4 text-xs text-ink-500 sm:flex-row sm:items-center">
          <p>© {new Date().getFullYear()} Levitron contributors. Released under the MIT license.</p>
          <p className="font-mono">
            cream <span className="text-ink-950">#fffdf8</span> · ink <span className="text-ink-950">#000000</span>
          </p>
        </div>
      </div>
    </footer>
